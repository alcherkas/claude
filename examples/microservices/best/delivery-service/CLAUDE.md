# CLAUDE.md — `delivery-service`

> Project-level agent context for **delivery-service**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Delivery assignment + tracking. Assigns an available driver, reads the order, emits DeliveryStatusChanged.

- **Port:** `8092`  ·  **Database:** `delivery_db`  ·  **Gateway route:** `/api/deliveries`
- **Stack:** Java 21 / Spring Boot 3.3.2 · base package `com.quickbite.delivery`
...
- **Calls (sync, Feign `/internal/**`):** `order-service`, `driver-service`
- **Called by (sync):** `notification-service`
- **Events produced:** `deliveries.events`
- **Events consumed:** —
