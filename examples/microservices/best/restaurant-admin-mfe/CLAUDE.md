# CLAUDE.md — `restaurant-admin-mfe`

> Project-level agent context for **restaurant-admin-mfe**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Restaurant owner console: menu, incoming orders, promotions.

- **Dev port:** `3005`  ·  **Module-Federation name:** `restaurantAdmin`
...
## Cross-repo coupling
- **Talks to (via gateway):** `restaurant-service`, `menu-service`, `order-service`, `promotion-service`
