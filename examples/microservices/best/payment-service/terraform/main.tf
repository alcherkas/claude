...
locals {
  ...
  image_uri = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/payment-service:${var.image_tag}"
}
...
# Dedicated Postgres database (payment_db) for this stateful service.
...
module "service" {
  ...
  environment = {
    ...
    CLIENTS_WALLET_URL   = "http://wallet-service.quickbite.internal:8090"
