# CLAUDE.md — `promotion-service`

> Project-level agent context for **promotion-service**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Promo codes: validation and redemption reservation.

- **Port:** `8087`  ·  **Database:** `promotion_db`  ·  **Gateway route:** `/api/promotions`
- **Stack:** Java 21 / Spring Boot 3.3.2 · base package `com.quickbite.promotion`
...
- **Called by (sync):** `pricing-service`
- **Events produced:** —
- **Events consumed:** —
