...
> Project-level agent context for **pricing-service**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Stateless price quote. POST /pricing/quote -> {subtotalCents, deliveryFeeCents, serviceFeeCents, taxCents, discountCents, totalCents, lineItems[]}.

- **Port:** `8086`  ·  **Database:** `_stateless_`  ·  **Gateway route:** `/api/pricing`
- **Stack:** Java 21 / Spring Boot 3.3.2 · base package `com.quickbite.pricing`
...
- **Calls (sync, Feign `/internal/**`):** `menu-service`, `promotion-service`
