# CLAUDE.md — `restaurant-service`

> Project-level agent context for **restaurant-service**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Restaurant profiles, status, geo. Validates owner against identity; emits restaurant.events.

- **Port:** `8082`  ·  **Database:** `restaurant_db`  ·  **Gateway route:** `/api/restaurants`
- **Stack:** Java 21 / Spring Boot 3.3.2 · base package `com.quickbite.restaurant`
...
- **Called by (sync):** `menu-service`, `search-service`, `order-service`, `review-service`
- **Events produced:** `restaurant.events`
- **Events consumed:** —
