...
> Project-level agent context for **notification-service**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Consumes the three operational topics; renders + (mock) sends EMAIL/SMS/PUSH. Coupling to order/delivery/payment is primarily via events.

- **Port:** `8093`  ·  **Database:** `notification_db`  ·  **Gateway route:** `/api/notifications`
- **Stack:** Java 21 / Spring Boot 3.3.2 · base package `com.quickbite.notification`
...
- **Calls (sync, Feign `/internal/**`):** `order-service`, `delivery-service`, `payment-service`
- **Called by (sync):** —
- **Events produced:** —
- **Events consumed:** `orders.events`, `payments.events`, `deliveries.events`
