...
> Project-level agent context for **shell**. Inherited **on top of** the workspace spine (`../CLAUDE.md`) and the canonical contract (`../PLATFORM_SPEC.md`). If anything here disagrees with the spec, **the spec wins**.

## What this repo is
Host application + auth shell. Loads the six feature remotes via Module Federation.

- **Dev port:** `3000`  ·  **Module-Federation name:** `shell`
...
## Cross-repo coupling
- **Talks to (via gateway):** gateway (auth), lazy-loads all remotes
