...
locals {
  ...
  image_uri = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/driver-service:${var.image_tag}"
}
...
# Postgres database for driver-service (driver_db).
