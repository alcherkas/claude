...
# Shared platform foundation (VPC, subnets, ECS cluster, ALB listener) exposed by platform-infra.
...
# Dedicated Postgres database (promotion_db) for this stateful service. The module appends "_db",
# so name = "promotion" yields the database "promotion_db".
# In a real polyrepo the source is pinned, e.g.:
...
module "service" {
  ...
  # Mirrors application.yml; values overridable via env at runtime.
  # DB_URL/DB_USERNAME/DB_PASSWORD are injected as container secrets from db_secret_arn, so they
  # must NOT also appear here (ECS rejects a key present in both environment and secrets).
  environment = {
