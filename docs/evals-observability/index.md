...
- [Langfuse](other/langfuse-observability-application-tracing.md) — MIT open-core (only `/ee` enterprise modules are commercial); OTel-native SDKs · [Langfuse evaluation](other/langfuse-evaluation-overview.md) *(existing pages; license verified this pass)*. **Native Claude Code integration is a hooks-based Marketplace plugin — not MCP/OTel** *(coding-agent-targeted; verified 2026-06-28)*
- [opentelemetry-hooks](other/opentelemetry-hooks.md) — open-source hook-based OTel layer spanning **all** major coding agents in one tool (Claude Code, Codex, Cursor, Gemini CLI, **GitHub Copilot**, OpenCode, Antigravity); the only verified option covering Copilot and Cursor, which lack first-party OTel/MCP eval-observability integrations *(coding-agent-targeted)*
...
- **Coding-agent-specific vs. general — substantially narrowed (verified 2026-06-28):** a dedicated coding-agent-integration pass confirmed *first-party* coding-agent integrations for the major eval/observability vendors via four distinct mechanisms: **vendor MCP servers** ([Braintrust](other/braintrust-eval-action.md) hosted US/EU server for ~10 clients; [LangSmith](other/langsmith-observability.md) `langsmith-mcp-server`), **agent-native OTel emission** (Claude Code and Gemini CLI emit metrics/logs/traces over OTLP), **hooks-based plugins** ([Langfuse](other/langfuse-observability-application-tracing.md)'s Claude Code plugin), and a **vendor-neutral OTel bridge** ([opentelemetry-hooks](other/opentelemetry-hooks.md)). DeepEval's MCP server remains confirmed too. **Remaining gap:** **GitHub Copilot and Cursor have no verified first-party eval/observability integration** — they are reachable only as MCP *clients* of vendor servers or via the third-party opentelemetry-hooks bridge.
